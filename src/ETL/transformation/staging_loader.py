...
def write_df_to_table(df_name: str, engine: Engine, df: pd.DataFrame, table_name: str, if_exists: str = "append", chunksize: int = 10000,) -> int:
    if df is None or df.empty:
        print(f"[staging_loader] for {table_name} is empty")
        return 0

    with engine.begin() as conn:
        print(f"[staging_loader] writing {df_name} to sql table -> {table_name}")
        df.to_sql(table_name, conn, if_exists=if_exists, index=False, chunksize=chunksize, method="multi")

    return len(df)

# load staging hdma table faster by using memory instead of df.to_sql-------------
def df_fast_loader(sample: bool = True,seed: int = 0,engine: Optional[Engine] = None,) -> None:
    """
...
def load_hdma_staging(sample: bool = True,seed: int = 0,engine: Optional[Engine] = None,) -> None:
    ...
    try:
        inserted_acc = write_df_to_table("HDMA_accepted", engine,accepted_stage,table_name="staging_accepted_hdma",if_exists="append", chunksize=100000)
        print(f"[staging_loader] Inserted {inserted_acc} rows inside staging_accepted_hdma")
    except Exception as e:
        print(f"Error in staging_loader.py -> write_df_to_table: {e}")
    ...
    try:
        print("writing df to sql table -> staging_rejected_hdma")
        inserted_rejec = write_df_to_table("HDMA_rejected", engine,rejected_stage,table_name="staging_rejected_hdma",if_exists="append", chunksize=10000)
        print(f"[staging_loader] Inserted {inserted_rejec} rows inside staging_rejected_hdma")
    except Exception as e:
        print(f"Error in staging_loader.py -> write_df_to_table: {e}")
...
if __name__ == "__main__":
    ...
    df_fast_loader(sample = False, engine = engine)
